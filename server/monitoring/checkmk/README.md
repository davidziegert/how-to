...
```bash
sudo wget -q https://download.checkmk.com/checkmk/2.4.0p24/check-mk-raw-2.4.0p24_0.noble_amd64.deb -O check-mk-latest.deb
```

2. Install the Checkmk package

```bash
sudo apt install check-mk-latest.deb
```
...
```bash
sudo wget -q http://your-server/monitoring/check_mk/agents/check-mk-agent_2.4.0p24_0.noble_amd64.deb -O check-mk-agent-latest.deb
```
